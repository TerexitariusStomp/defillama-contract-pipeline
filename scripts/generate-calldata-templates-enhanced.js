...
function extractABIFromAdapterWithHelpers(adapterPath) {
  ...
  console.log(`Extracting ABI from: ${adapterPath}`);
  ...
  // Set to collect all ABI definitions
  ...
  // Function to add ABI to the collection
  ...
  function addABI(abi) {
    ...
    if (Array.isArray(abi)) {
      ...
      // Object format: keys are function names



      Object.keys(abi).forEach(name => {
   ...
   // Try to extract ABI from adapter file (including helper modules)
   ...
   try {
     ...
     console.log(`Processing ${contract.protocol_slug}, adapter_file: ${contract.adapter_file}`);
     ...
     if (fs.existsSync(adapterPath)) {
       ...
       console.log(`[${contract.protocol_slug}]: extracted ${abi.length} functions`);
     ...
     console.log(`Error processing ${contract.protocol_slug}: ${e.message}`);
  ...
  if (!abiMatch) {
    ...
    abiMatch = adapterContent.match(/abi\s*:\s*(\[[^\]]+\]|\{[^%s]+\})\s*[;,]?/s);
  ...
  if (abiMatch) {
    ...
    try {
      ...
      console.log(`  Found direct ABI with ${Array.isArray(abi) ? abi.length : Object.keys(abi).length} functions`);
      ...
      console.log(`  Failed to parse direct ABI: ${e.message}`);
  ...
  // Now, try to find and parse imported modules that might contain ABI
  ...
  while ((requireMatch = requirePattern.exec(adapterContent)) !== null) {
    ...
    if (modulePath.startsWith('.')) {
      ...
      // Relative path from the adapter file's directory
      ...
      resolvedPath = path.resolve(adapterDir, modulePath);
      ...
      // Try to resolve as a node_modules module
    ...
    if (fs.existsSync(resolvedPath) && !importedFiles.includes(resolvedPath)) {
      ...
      console.log(`  Found imported module: ${resolvedPath}`);
      ...
      try {
        ...
        // Search for ABI in the imported file



        let importedABIMatch = importedContent.match(/abi\s*[=:]\s*(\[[^\]]+\]|\{[^%s]+\})\s*[;,]?/s);
        ...
        if (!importedABIMatch) {
          ...
          importedABIMatch = importedContent.match(/abi\s*:\s*(\[[^\]]+\]|\{[^%s]+\})\s*[;,]?/s);
        ...
        if (importedABIMatch) {
          ...
          try {
            ...
            const importedABI = JSON.parse(importedABIMatch[1]);
            ...
            console.log(`    Found ABI in imported module with ${Array.isArray(importedABI) ? importedABI.length : Object.keys(importedABI).length} functions`);
            ...
            console.log(`    Failed to parse ABI from imported module: ${e.message}`);
        ...
        console.log(`    Error reading imported file: ${err.message}`);
  ...
  allABIs.forEach(func => {



    if (func.name && !seenNames.has(func.name)) {



      uniqueABIs.push(func);
  ...
  const template = {
    ...
    protocol: contract.protocol_slug,
  ...
  if (template.abi && template.abi.length > 0) {
    ...
    console.log(`[INFO] Generated calldata for ${contract.protocol_slug}`);
...
function generateCalldata(func, args) {
  try {
    const inputs = (func.inputs || []).map((input, index) => ({
      ...
      type: input.type,
      value: args[index] || `0x${'00'.repeat(32)}`
    }));
    ...
    inputs.forEach(input => {
      if (input.type === 'address') {
        calldata += `0x${input.value.substring(2).padStart(64, '0')}`;
      } else if (input.type === 'uint256') {
        calldata += `0x${parseInt(input.value).toString(16).padStart(64, '0')}`;
      } else {
        calldata += `0x${'00'.repeat(64)}`; // Default for unknown types
      }
    });
...
function main() {
  ...
  contracts.forEach((contract, index) => {
    try {
      ...
      successCount++;
      console.log(`[${index + 1}/${contracts.length}] Generated template for ${contract.protocol_slug} (${contract.chain_name})`);
    } catch (error) {
